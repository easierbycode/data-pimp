...
<script lang="ts">
	import {
		...
		VIDEOS,
		type Video,
	} from "../islands/dashboard-data.ts";
	...
	const kpi = $derived(derived.kpi);
	const products = $derived(derived.products);
	const videos = $derived(derived.videos);
	const scopeColor = $derived(derived.scopeColor);
