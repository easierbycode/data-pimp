  ...
  <script type="module">
    ...
    const api = {
      entities: {
        Sample: {
          list: async (orderBy) => {
            const res = await fetch(\`\${API_BASE}/api/samples\${orderBy ? \`?order_by=\${orderBy}\` : ''}\`);
        ...
        Bundle: {
          list: async (orderBy) => {
            const res = await fetch(\`\${API_BASE}/api/bundles\${orderBy ? \`?order_by=\${orderBy}\` : ''}\`);
            return res.ok ? res.json() : [];
          },
          filter: async (filters) => {
            const params = new URLSearchParams(filters);
            const res = await fetch(\`\${API_BASE}/api/bundles?\${params}\`);
            return res.ok ? res.json() : [];
          },
          create: async (data) => {
          ...
          delete: async (id) => {
            await fetch(\`\${API_BASE}/api/bundles/\${id}\`, { method: 'DELETE' });
          }
        },
...
Deno.serve(async (req) => {
  ...
  // API routes placeholder (for future backend implementation)
  if (pathname.startsWith("/api/")) {
    // Return empty arrays/objects for now - replace with real data layer
